...
class TestThermal:
    ...
    def test_chassis_thermal(self):
        ...
        for thermal in thermal_list:
            ...
            elif 'PMIC' in thermal.get_name():
                start_index = pmic_thermal_rule.get('start_index', 1)
                start_index += pmic_thermal_count
                assert thermal.get_name() == pmic_thermal_rule['name'].format(start_index)
                assert pmic_thermal_rule['temperature'].format(start_index) in thermal.temperature
                assert pmic_thermal_rule['high_threshold'].format(start_index) in thermal.high_threshold
                assert pmic_thermal_rule['high_critical_threshold'].format(start_index) in thermal.high_critical_threshold
                pmic_thermal_count += 1

        assert gearbox_thermal_count == 2
        assert cpu_thermal_count == 2
        assert sodimm_thermal_count == 2
        assert pmic_thermal_count == 2
