...
class TestDeviceData:
    ...
    @mock.patch('sonic_platform.device_data.time.sleep', mock.MagicMock())
    @mock.patch('sonic_platform.device_data.DeviceDataManager.get_sfp_count', mock.MagicMock(return_value=2))
    @mock.patch('sonic_platform.device_data.utils.read_int_from_file', mock.MagicMock(return_value=1))
    @mock.patch('sonic_platform.device_data.utils.wait_until_conditions')
    @mock.patch('sonic_platform.device_data.os.path.exists')
    @mock.patch('sonic_platform.device_data.DeviceDataManager.is_module_host_management_mode')
    def test_wait_platform_ready_lambda_binding(self, mock_is_indep, mock_exists, mock_wait_until):
        """Test that lambda functions correctly bind loop variables (idx and node)"""
        mock_exists.return_value = True
        mock_is_indep.return_value = False
        mock_wait_until.return_value = True

        DeviceDataManager.wait_platform_ready()

        # Get the conditions that were passed to wait_until_conditions
        args, kwargs = mock_wait_until.call_args
        conditions = args[0]

        # Expected: (2 SFPs * 6 sysfs_nodes) = 12
        assert len(conditions) == 12

        # Execute each condition and verify correct paths are checked
        for condition in conditions:
            condition()

        # Verify os.path.exists was called with correct paths for each sfp_index and sysfs_node
        sysfs_nodes = ['power_mode', 'power_mode_policy', 'present', 'reset', 'status', 'statuserror']
        expected_calls = []
        for sfp_index in range(2):
            for sysfs_node in sysfs_nodes:
                expected_calls.append(mock.call(f'/sys/module/sx_core/asic0/module{sfp_index}/{sysfs_node}'))
